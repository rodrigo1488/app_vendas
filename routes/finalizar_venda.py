...
def finalizar_venda():
    ...
    try:
        ...
        end_imp=data.get("end_impressora")
        ...
        imprimir_venda(meio_pagamento, valor_total, numeromesa,horario_fechamento, produtos_filtrados, end_imp)
...
def imprimir_venda(meio_pagamento, valor_total, numeromesa, horario_fechamento, produtos_filtrados, end_imp):
    """Envia os dados de fechamento para a impressora térmica usando ESC/POS"""
    try:
        impressora_ip = end_imp # IP da impressora
        ...
        for prod in produtos_filtrados:
            nome = (prod.get("nomeproduto") or "")[:20]
            qtd = prod.get("quantidade") or 0
            total = prod.get("valortotal_item") or 0.0

            try:
                p.text(f"{nome:<20}{qtd:>5}{float(total):>10.2f}\n")
            except Exception as err:
                print(f"[ERRO] Produto inválido para impressão: {prod} - {err}")
